...
int virtualCarInit()
{
	...
	//Level 2 Food list update

	int rowOfFoodList = sizeof(food_list) / sizeof(food_list[0]);
	...
	for (int i = 0; i < rowOfFoodList; ++i)
	{
		...
		int tmp = food_list[i][0];
		food_list[i][0] = food_list[i][1];
		food_list[i][1] = tmp;
	}

	FindMazeSolution(currentCell[1], currentCell[0], map, food_list, rowOfFoodList, instructions);
